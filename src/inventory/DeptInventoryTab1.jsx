...
export default function DeptInventoryTab() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      const categorized = logs.map(item => {
        ...
        const biochemNames = [
          "ALBUMIN/5 PACK/90 SLIDES", "ALKP/5 PACK/300SLIDES", "TOT BILIRUBIN/5 PACK/300 SLIDES",
          "BUBC/5 PACK/300 SLIDES (DIRECT AND INDIRECT BILIRUBIN)",
           "GLUCOSE/5 PACK/300 SLIDES",
          "BUN/5 PACK/300 SLIDES", "CALCIUM/5 PACK/300 SLIDES", "CL/5 PACK/250 SLIDES",
          "CHOLESTROL /5 PACK/300 SLIDES", "CREATININE/5 PACK/300 SLIDES", "CRP/5 PACK/90 SLIDES",
          "DIRECT LDL/5 PACK/90 SLIDES",
          "GGT/5 PACK/250 SLIDES",
          "HDL/5 PACK/90 SLIDES",
          "LDH/5 PACK/250 SLIDES",
          "PHOSPHORUS/5 PACK/300 SLIDES", "POTASSIUM/K+/5 PACK/250 SLIDES",
          "AST/5 PACK/300 SLIDES", "ALT/5 PACK/300 SLIDES", "SODIUM/Na+/5 PACK/250 SLIDES",
          "IRON/5 PACK/90 SLIDEST", "TOTAL PROTEIN/5 PACK/90 SLIDES", "TRIGLYCERIDES/5 PACK/90 SLIDES",
          "URIC ACID/5 PACK/300 SLIDES", "DTIBC REAGENT BOX/ 300 TEST"
        ].map(n => n.toUpperCase());
  ...
  const storageReagents = useMemo(() => inventory.filter(item =>
    ...
    console.log(
      "STORAGE REAGENTS:",
      storageReagents.map(r => r.reagentName)
    );
  ...
  const handleMarkConsumed = async (item) => {
    ...
    if (item.isConsumable) {
      ...
      if (!window.confirm(`Mark ${item.reagentName} packet as consumed?`)) {
        return;
      }
  ...
  item.inventoryUnit === "ML"
    ...
    : Number(item.totalTests || 0);
      ...
      if (remainingQty > 0) {
        if (!window.confirm(`${unit} are still remaining. Mark as     Consumed?`)) return;
        }
...
batch.update(
  ...
  item?.inventoryUnit === "ML"
    ...
    : {
      ...
      await addConsumptionLedgerEntry({
        productName: item?.reagentName || "Unknown",
    ...
    try {
      ...
      for (const name of selections) {
        ...
        if (item) {
          ...
          const currentQty = item.inventoryUnit === "ML"
    ? Number(item.totalML || 0)
    : Number(item.totalTests || 0);
...
batch.update(
    ...
    try {
      ...
      for (const name of selections) {
        ...
        batch.update(
          ...
          batchNumbers.push(item.lotNo || "N/A");
  ...
  <div className="active-reagents-list">
    ...
    {activeConsumables.map(item => (

      <div key={item.id} className="reagent-health-item">
            ...
            <div className="cal-modal-box" style={{ width: '500px', maxHeight: '90vh', overflowY: 'auto'}}>
              ...
              <div className="cal-form-group">
                ...
                <select value={qcReason} onChange={(e) => setQCReason(e.target.value)}>
                  ...
                  <option value="NEW LOT OF REAGENT">NEW LOT OF REAGENT</option>
