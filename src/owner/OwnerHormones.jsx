...
export default function OwnerHormones() {
  ...
  useEffect(() => {
    const unsub = subscribeOverview({
      source,
      dateRange,
      onData: ({ unifiedRows,kpis,staffAnalytics,}) => {
  ...
  const stackedChartSLA = useMemo(() => {
    const dept = testTimings?.hormones;
    ...
    switch (stageFilter) {
      ...
      case "collected":
        return dept.collected_to_scanned ?? null;
  
      case "saved":
        return dept.scanned_to_saved ?? null;
  
        case "validated":
          return dept.saved_to_validated ?? null;
        
        case "entered":
          return dept.validated_to_entered ?? null;
        
        case "turnaround":
          return dept.turnaround ?? null;
        
        case "complete":
          return dept.complete_analysis ?? null;
