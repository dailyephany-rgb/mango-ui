...
// ------------------------------------------------------
// Blood Group Testing — STRICT Time Printed Implementation
// ------------------------------------------------------
...
export function subscribeOverview({ onData, source = "All", dateRange }) {
  ...
  const bgRef = scopedTimePrintedQuery("bloodgroup_testing_register", dateRange);
  if (!masterRef || !bgRef) {
    const empty = () => {};
    empty.updateSource = () => {};
    return empty;
  ...
  const runPublish = () => {
    ...
    onDataLive({masterRows:filteredMaster,
